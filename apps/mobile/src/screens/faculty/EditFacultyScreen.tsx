...
export function EditFacultyScreen({ navigation, route }: Props) {
  ...
  const eligibleTeachers = staffList.filter(
    (m) => m.roles.includes("teacher") && m.isActive && (!m.linkedFaculty || m.linkedFaculty.id === faculty.id)
  );
                ...
                {createOpen && (
                  <View style={s.createForm}>
                    <FormField label="EMAIL ADDRESS" value={newEmail} onChangeText={setNewEmail}
                      placeholder="e.g. priya@institute.com" keyboardType="email-address" icon="mail-outline" required />
                    <FormField label="PHONE NUMBER" value={newPhone} onChangeText={(v) => setNewPhone(v.replace(/\D/g, ""))}
                      placeholder="e.g. 9876543210" keyboardType="phone-pad" icon="call-outline" required />
...
const makeSStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  toggleLabel:   { ...T.chipText, color: C.text },
  toggleSub:     { ...T.caption, color: C.muted, marginTop: ms(2) },
  ...
  linkedName:  { ...T.listItemTitle, color: C.text },
  linkedSub:   { ...T.caption, color: C.muted, marginTop: ms(1) },
  unlinkT:     { ...T.chipText, color: C.red },
  ...
  pwLabel:    { ...T.chipText, color: C.text, marginBottom: ms(7) },
  pwRow:      { flexDirection: "row", alignItems: "center", gap: ms(8), backgroundColor: C.inputBg, borderRadius: ms(12), borderWidth: StyleSheet.hairlineWidth, borderColor: C.border, paddingHorizontal: ms(14), paddingVertical: ms(12), marginBottom: ms(6) },
  pwInput:    { flex: 1, ...T.body, color: C.text, padding: 0 },
  pwHint:     { ...T.caption, color: C.muted, marginBottom: ms(16) },
